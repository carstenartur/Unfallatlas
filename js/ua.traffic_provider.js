...
(function initTrafficProvider(root, factory) {
...
})(typeof window !== 'undefined' ? window : null, function createTrafficProviderApi(sourceManifest) {
  'use strict';

  if (!sourceManifest || typeof sourceManifest.normalizeManifest !== 'function') {
    throw new Error('ua.source_manifest.js must be loaded before ua.traffic_provider.js');
  }
  ...
  function normalizeDescriptor(value) {
    ...
    if (measurementType === 'proxy' && unit) {
      fail('proxy_numeric_unit_forbidden', 'proxy descriptors must not declare a numeric traffic unit');
    }
    if (measurementType !== 'proxy' && !unit) {
      fail('missing_unit', `${measurementType} descriptors require an explicit unit`);
    }
    ...
    const source = validateSourceRecord({ ...value, id: requiredString(value.id, 'id') });
  ...
  function normalizeObservation(value, descriptor, index) {
    ...
    return Object.freeze({
      ...
      geometry,
      wayId,
      direction: value.direction == null ? null : requiredString(value.direction, `${path}.direction`),
      qualityNotes: Object.freeze(Array.isArray(value.qualityNotes)
        ? value.qualityNotes.map((item, noteIndex) => requiredString(item, `${path}.qualityNotes[${noteIndex}]`))
        : []),
      source: descriptor,
    });
  }
  ...
  function createProvider(options) {
    ...
    if (typeof opts.loadObservations !== 'function') {
      fail('invalid_provider', `${descriptor.id} must implement loadObservations(context)`);
    }
      ...
      async loadObservations(context) {
        ...
        for (const observation of observations) {
          if (ids.has(observation.observationId)) {
            fail('duplicate_observation', `${descriptor.id} returned duplicate ${observation.observationId}`);
          }
    ...
    return Object.freeze({
      register(provider) {
        if (!provider || !provider.descriptor || typeof provider.canProvide !== 'function' ||
          ...
          fail('invalid_provider', 'provider does not implement the traffic contract');
        }
        if (entries.has(provider.id)) fail('duplicate_provider', `provider ${provider.id} already registered`);
  ...
  function localProjection(origin) {
    const metersPerLon = 111320 * Math.max(0.01, Math.abs(Math.cos(origin.lat * Math.PI / 180)));
    return coordinate => ({
  ...
  function matchObservationToRoads(observation, roads, options) {
    ...
    if (observation.wayId) {
      const exact = roads.find(road => String(road.wayId) === observation.wayId);
      if (exact) {
        return Object.freeze({
          observation,
          road: exact,
  ...
  function evidenceRank(match, referenceYear, maxFreshAgeYears) {
    ...
    const ageYears = Math.max(0, referenceYear - match.observation.year);
  ...
  function selectTrafficEvidence(matches, options) {
    ...
    const candidates = (Array.isArray(matches) ? matches : [])
      ...
      .map(match => ({
        ...match,
        ageYears: Math.max(0, referenceYear - match.observation.year),
  ...
  function createOsmProxyObservation(input) {
    ...
    return {
      ...
      qualityNotes: ['OSM-Straßenklassenproxy; kein Messwert und kein DTV-Schätzwert.'],
    };
  }
