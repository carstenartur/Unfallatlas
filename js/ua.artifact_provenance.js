...
(function initArtifactProvenance(root, factory) {
  ...
  function createArtifactProvenanceApi(sourceManifest) {
    "use strict";
    ...
    if (
      ...
      throw new Error(
        "ua.source_manifest.js must be loaded before ua.artifact_provenance.js",
      );
    }
    ...
    function bytes(value) {
      ...
      fail(
        "invalid_bytes",
        "artifact content must be UTF-8 text, Buffer or Uint8Array",
      );
    }
    ...
    async function sha256(value) {
      ...
      if (!subtle)
        fail("sha256_unavailable", "Web Crypto SHA-256 is unavailable");
    ...
    function normalizeSourceIds(value, manifest, path, options = {}) {
      ...
      if (!Array.isArray(supplied))
        fail("invalid_source_ids", `${path} must be an array`);
      const ids = [
        ...new Set(
      ...
      if (!ids.length && options.defaultAll)
        return manifest.sources.map((source) => source.sourceId);
      if (!ids.length && !options.allowEmpty)
        fail("missing_source_ids", `${path} must not be empty`);
      const known = sourceIdSet(manifest);
      const unknown = ids.filter((id) => !known.has(id));
      if (unknown.length)
        fail("unknown_source_id", `${path} contains unknown IDs`, { unknown });
      return ids;
    }
    ...
    function compactSourceNotice(manifestValue, options = {}) {
      ...
      const labels = summaries.map(
        (source) => `${source.label} – ${source.licenseLabel}`,
      );
    ...
    async function buildSidecar(options) {
      ...
      const artifactMediaType = requiredString(
        opts.artifactMediaType,
        "artifactMediaType",
      );
      const artifactBytes =
        opts.artifactBytes == null ? null : bytes(opts.artifactBytes);
      const artifactSha256 = opts.artifactSha256
        ? requiredString(opts.artifactSha256, "artifactSha256").toLowerCase()
        : artifactBytes
          ? await sha256(artifactBytes)
          : null;
      ...
      const sidecar = {
        ...
        visibleSourceNotice: compactSourceNotice(
          normalized.manifest,
          opts.noticeOptions,
        ),
      };
    ...
    function cloneJson(value, path) {
      try {
        ...
        fail("invalid_json", `${path} is not JSON-serializable`, {
    ...
    async function attachGeoJsonProvenance(
      ...
      const defaultIds = normalizeSourceIds(
        options.defaultSourceIds,
        normalized.manifest,
        "defaultSourceIds",
        { defaultAll: true },
      );
      ...
      if (
        collection.metadata.sourceManifest ||
        collection.metadata[MANIFEST_HASH_PROPERTY]
      ) {
        fail(
          "existing_provenance",
          "GeoJSON already contains provenance metadata",
        );
      }
...
`,
    ...
    async function buildKmlExtendedData(manifestValue, options = {}) {
      ...
      const fields = [
        ...
        kmlData(
          "unfallatlas:sourceManifestSchemaVersion",
          normalized.manifest.schemaVersion,
        ),
        kmlData(
          "unfallatlas:sourceIds",
          normalized.manifest.sources
            .map((source) => source.sourceId)
            .join(","),
        ),
        kmlData(
          "unfallatlas:sourceNotice",
          compactSourceNotice(normalized.manifest, options.noticeOptions),
        ),
    ...
    async function injectKmlProvenance(kmlValue, manifestValue, options = {}) {
      ...
      if (
        /<ExtendedData\b[^>]*>[\s\S]*unfallatlas:sourceManifestSha256/i.test(
          kml,
        )
      ) {
        fail(
          "existing_provenance",
          "KML already contains Unfallatlas provenance",
        );
      }
