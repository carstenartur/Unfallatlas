...
(function initSourceManifest(root, factory) {
  ...
  function createSourceManifestApi() {
    "use strict";
    ...
    function validDate(value, path) {
      ...
      if (!match) {
        fail(
          ...
          path,
          value,
          `${path} must be an ISO-8601 date or timestamp with timezone`,
        );
      }
    ...
    function sha256(value, path, optional) {
      ...
      if (!/^[a-f0-9]{64}$/.test(text)) {
        fail(
          ...
          path,
          value,
          `${path} must be a 64-character SHA-256 hex digest`,
        );
      }
      return text;
    }
    ...
    function stringArray(value, path, options) {
      ...
      if (!Array.isArray(value))
        fail("invalid_array", path, value, `${path} must be an array`);
    ...
    function canonicalLicenseId(value, path) {
      ...
      if (!canonicalKey || !LICENSE_POLICIES[canonicalKey]) {
        fail(
          ...
          path,
          value,
          `${path} is not in the open-licence allowlist`,
        );
      }
    ...
    function normalizeBounds(value, path) {
      ...
      for (const key of ["south", "west", "north", "east"]) {
        ...
        if (!Number.isFinite(number))
          fail("invalid_coordinate", `${path}.${key}`, bounds[key]);
      ...
      if (
        ...
        fail(
          ...
          path,
          value,
          `${path} is not a valid geographic bounding box`,
        );
      }
      return normalized;
    }
        ...
        ...(scenario.bounds == null
          ...
          : {
              bounds: normalizeBounds(
                scenario.bounds,
                "manifest.scenario.bounds",
              ),
            }),
        filters: cloneJson(
          objectValue(scenario.filters || {}, "manifest.scenario.filters"),
          "manifest.scenario.filters",
        ),
        ...(years.length
    ...
    function normalizePermissions(value, path) {
      ...
      if (
        normalized.permitsRedistribution === false ||
        ...
        fail(
          ...
          path,
          permissions,
          `${path} does not permit standard distribution and derivatives`,
        );
      }
      return normalized;
    }
    ...
    function normalizeSource(value, index) {
      ...
      if (!SOURCE_ROLES.includes(role))
        fail("invalid_role", `${path}.role`, role);
      ...
      if (typeof changedOrDerived !== "boolean") {
        fail(
          "missing_derivation_flag",
          `${path}.changedOrDerived`,
          changedOrDerived,
          `${path}.changedOrDerived must explicitly be true or false`,
        );
      }
      const requiredAttribution = optionalString(
        source.requiredAttribution,
        `${path}.requiredAttribution`,
      );
      ...
      const distributionUrl = httpsUrl(
      ...
      return {
        ...
        ...(source.temporalCoverage
          ? {
              temporalCoverage: requiredString(
                source.temporalCoverage,
        ...
        ...(source.spatialCoverage
          ? {
              spatialCoverage: requiredString(
                source.spatialCoverage,
        ...
        ...(source.contentHash
          ? { contentHash: sha256(source.contentHash, `${path}.contentHash`) }
          : {}),
        changedOrDerived,
    ...
    function cloneJson(value, path) {
      ...
      return Object.fromEntries(
        Object.keys(object)
          .sort()
          .map((key) => [key, cloneJson(object[key], `${path}.${key}`)]),
      );
    }
    ...
    function sourceIdsForFields(manifestValue, fields) {
      ...
      const requested = new Set(
        stringArray(fields, "fields", { allowEmpty: true }),
      );
      const ids = new Set();
