 ...
 *
 * A PNG is never downloaded on its own. The browser captures the fully rendered
 * map, appends a visible source strip derived from the shared SourceManifest,
 * hashes the final PNG bytes and emits one ZIP containing the image, its exact
 * .sources.json sidecar and a human-readable README.
 */
(function initStaticMapExportProvenance(root, factory) {
  ...
  if (root) {
    ...
    try {
      ...
      root.console?.error?.("PNG-Export-Provenienz konnte nicht initialisiert werden", error);
    }
  }
})(typeof window !== "undefined" ? window : null, function createStaticMapExportProvenanceApi(
  artifactProvenance,
  zip,
) {
  "use strict";
  ...
  function requireDependencies() {
    if (!artifactProvenance?.normalizeAndHash || !artifactProvenance?.buildMediaProvenance) {
  ...
  function bytes(value, path = "bytes") {
  ...
  function validatePngBytes(value, path = "png") {
    const content = bytes(value, path);
  ...
  function decodeBase64(value, environment = {}) {
    const rootValue = environment.root || (typeof window !== "undefined" ? window : null);
  ...
  function pngBytesFromDataUrl(value, environment = {}) {
    ...
    if (!match) {
      fail("invalid_data_url", "PNG capture must use an image/png base64 data URL");
    }
    return validatePngBytes(decodeBase64(match[1], environment), "captured PNG");
  }
  ...
  function measure(context, text) {
    ...
    return Number.isFinite(measured) ? measured : String(text).length * 7;
  }
  ...
  function wrapText(context, value, maxWidth, maxLines) {
    ...
    for (const word of words) {
      ...
      if (!current || measure(context, candidate) <= maxWidth) {
        current = candidate;
        continue;
      }
      lines.push(current);
    ...
    if (represented.length < sourceText.length && lines.length) {
      lines[lines.length - 1] = ellipsize(context, lines[lines.length - 1], maxWidth);
    }
  ...
  function defaultLoadImage(rootValue, dataUrl) {
    ...
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new StaticMapExportError(
        "image_decode_failed",
  ...
  function verifySourceStrip(context, sourceHeight, outputHeight, width) {
    if (typeof context?.getImageData !== "function") return;
    const border = context.getImageData(Math.min(2, width - 1), sourceHeight, 1, 1).data;
    ...
    const darkBorder = border[0] < 90 && border[1] < 90 && border[2] < 90 && border[3] > 200;
    const lightBackground =
      background[0] > 220 && background[1] > 220 && background[2] > 220 && background[3] > 200;
    if (!darkBorder || !lightBackground) {
      fail("source_strip_verification_failed", "Rendered PNG source strip failed pixel verification");
    }
  }

  async function renderPngWithSourceStrip(options) {
    const opts = options || {};
    const rootValue = opts.root || (typeof window !== "undefined" ? window : null);
    pngBytesFromDataUrl(opts.sourceDataUrl, { root: rootValue });
    const image = await (opts.loadImage || defaultLoadImage)(rootValue, opts.sourceDataUrl);
    const sourceWidth = Number(image?.naturalWidth || image?.width);
    const sourceHeight = Number(image?.naturalHeight || image?.height);
    if (!Number.isInteger(sourceWidth) || !Number.isInteger(sourceHeight) || sourceWidth < 64 || sourceHeight < 64) {
      fail("invalid_dimensions", "Captured map PNG has invalid dimensions", {
        width: sourceWidth,
        height: sourceHeight,
      });
    }
    if (sourceWidth > 16384 || sourceHeight > 16384 || sourceWidth * sourceHeight > 120000000) {
      fail("unsafe_dimensions", "Captured map PNG exceeds the safe rendering budget");
    }

    const canvas = (opts.canvasFactory || defaultCanvasFactory)(rootValue);
    const context = canvas.getContext?.("2d", { alpha: false });
    if (!context) fail("canvas_context_unavailable", "2D canvas context is unavailable");
    ...
    const maxTextWidth = Math.max(40, sourceWidth - horizontalPadding * 2);
    context.font = `600 ${fontSize}px system-ui, sans-serif`;
    const noticeLines = wrapText(context, opts.visibleNotice, maxTextWidth, 3);
    context.font = `400 ${Math.max(11, fontSize - 1)}px ui-monospace, monospace`;
    const hashLines = wrapText(
      context,
      `SourceManifest SHA-256: ${String(opts.sourceManifestSha256 || "")}`,
      maxTextWidth,
      2,
    );
    if (!noticeLines.length || !hashLines.length) {
      fail("missing_source_notice", "Visible source notice and manifest hash are required");
    }
    const allLines = [...noticeLines, ...hashLines];
    const stripHeight = 3 + horizontalPadding * 2 + allLines.length * lineHeight;
    const outputHeight = sourceHeight + stripHeight;
    if (outputHeight > 16384) fail("unsafe_dimensions", "PNG plus source strip exceeds canvas limits");
    ...
    let y = sourceHeight + horizontalPadding;
    context.fillStyle = "#111827";
    context.font = `600 ${fontSize}px system-ui, sans-serif`;
    for (const line of noticeLines) {
      context.fillText(line, horizontalPadding, y, maxTextWidth);
      y += lineHeight;
    }
  ...
  function buildReadme(options) {
    const opts = options || {};
    const lines = artifactProvenance.visibleSourceLines(opts.manifest);
    return (
  ...
  async function buildPngPackage(options) {
    ...
    const noticeOptions = {
      maxCharacters: Number(opts.noticeOptions?.maxCharacters) || DEFAULT_NOTICE_CHARACTERS,
      ...(opts.noticeOptions?.prefix ? { prefix: opts.noticeOptions.prefix } : {}),
    };
    ...
    const renderedValue = await renderer({
    ...
    const pngBytes = validatePngBytes(renderedValue?.bytes || renderedValue, "rendered PNG");
    ...
    if (media.visibleNotice !== visibleNotice) {
      fail("notice_mismatch", "Rendered source notice differs from the sidecar notice");
    }
    const sidecar = JSON.parse(media.sidecarJson);
    if (sidecar.sourceManifestSha256 !== normalized.sha256) {
      fail("manifest_hash_mismatch", "Sidecar does not bind the rendered manifest");
    }
    ...
    const entries = Object.freeze([
      Object.freeze({ name: artifactName, mediaType: PNG_MEDIA_TYPE, content: pngBytes }),
      Object.freeze({
  ...
  function heatmapExportOpacity(rootValue) {
    const element = rootValue?.document?.getElementById?.("heatExportOpacity");
    const value = Number(element?.value);
    return Number.isFinite(value) ? Math.max(0, Math.min(100, value)) / 100 : 0.4;
  }

  function download(rootValue, content, filename) {
    const blob = new rootValue.Blob([content], { type: PACKAGE_MEDIA_TYPE });
  ...
  async function exportCurrentMap(UA, rootValue, ctxValue, options = {}) {
    requireDependencies();
    if (!UA || !rootValue) fail("invalid_environment", "Browser UA and window are required");
    const ctx = ctxValue || UA.getRuntimeContext?.();
    if (!ctx || typeof ctx !== "object") fail("missing_context", "Runtime map context is unavailable");
    if (typeof UA.captureExportMapImage !== "function") {
      fail("missing_capture", "Readiness-checked map capture is unavailable");
    }
    const createManifest = UA.exportProvenanceRuntime?.createManifest;
    if (typeof createManifest !== "function") {
      fail("missing_manifest_runtime", "Live SourceManifest runtime is unavailable");
    }

    const manifestBefore = await createManifest(ctx, { UA, root: rootValue });
    const before = await artifactProvenance.normalizeAndHash(manifestBefore);
    const sourceDataUrl = await UA.captureExportMapImage(ctx, {
      heatmapExportOpacity: options.heatmapExportOpacity ?? heatmapExportOpacity(rootValue),
    });
    const manifestAfter = await createManifest(ctx, { UA, root: rootValue });
    const after = await artifactProvenance.normalizeAndHash(manifestAfter);
    ...
    const result = await buildPngPackage({
      ...
      sourceDataUrl,
      artifactName: options.artifactName || artifactNameForManifest(before.manifest),
    ...
    if (options.download !== false) download(rootValue, result.archive, result.packageFileName);
    return result;
  }
  ...
  function ensureUi(rootValue) {
    ...
    let container = documentValue.getElementById("staticMapExportContainer");
    if (container) {
      return {
        container,
        button: documentValue.getElementById("btnExportPNG"),
        progress: documentValue.getElementById("staticMapExportProgress"),
      };
    }
    ...
    container = createElement(documentValue, "fieldset", {
      ...
      style: "margin-top:10px;padding:8px 12px;border:1px solid rgba(0,0,0,.12);border-radius:12px;",
    });
    ...
    const button = createElement(documentValue, "button", {
      id: "btnExportPNG",
      type: "button",
      "aria-label": "Kartenbild mit vollständiger Quellenprovenienz exportieren",
      textContent: "🗺️ Karte + Quellen (ZIP)",
      style: "width:100%;padding:10px 16px;background:#365f91;color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-size:13px;",
    });
    const explanation = createElement(documentValue, "div", {
      textContent: "Enthält PNG, sichtbare Quellenleiste und hashgebundenes SourceManifest-Sidecar.",
      style: "margin-top:6px;font-size:11px;color:#666;line-height:1.4;",
    });
    const progress = createElement(documentValue, "div", {
      id: "staticMapExportProgress",
      role: "status",
      "aria-live": "polite",
      style: "display:none;margin-top:8px;font-size:12px;color:#555;",
    });
    container.append(legend, button, explanation, progress);

    const videoContainer = documentValue.getElementById("videoExportContainer");
    const dataContainer = documentValue.getElementById("exportGroupData");
    if (videoContainer?.parentNode) videoContainer.parentNode.insertBefore(container, videoContainer);
    else if (dataContainer?.parentNode) dataContainer.parentNode.insertBefore(container, dataContainer.nextSibling);
    else documentValue.body.appendChild(container);
    return { container, button, progress };
  }

  function reportFailure(UA, rootValue, progress, error) {
    rootValue.console?.error?.("PNG-Kartenexport mit Quellenprovenienz fehlgeschlagen", error);
    if (progress) {
      progress.textContent = `❌ Export abgebrochen: ${error?.message || String(error)}`;
      progress.style.display = "";
    }
    if (typeof UA.showToast === "function") {
      UA.showToast("PNG-Export abgebrochen: Karte und Quellen konnten nicht konsistent erzeugt werden.");
    }
    if (typeof rootValue.CustomEvent === "function" && rootValue.dispatchEvent) {
      rootValue.dispatchEvent(new rootValue.CustomEvent("unfallwerkbank:export-error", {
        detail: { code: error?.code || "png_export_failed", message: error?.message || String(error) },
      }));
    }
  }
  ...
  function install(UA, rootValue) {
    requireDependencies();
    if (!UA || !rootValue?.document) return Object.freeze({ available: false });
    if (UA.__staticMapExportProvenanceInstalled) return UA.staticMapExportProvenanceRuntime;
    if (typeof rootValue.leafletImage !== "function" || typeof UA.captureExportMapImage !== "function") {
      return Object.freeze({ available: false, reason: "map_capture_unavailable" });
    }
    if (typeof UA.exportProvenanceRuntime?.createManifest !== "function") {
      fail("missing_manifest_runtime", "Export provenance must load before static map export");
    }

    const ui = ensureUi(rootValue);
    if (!ui.button) fail("missing_button", "Static map export button could not be created");
    ui.button.addEventListener("click", async () => {
      ui.button.disabled = true;
      ui.button.style.opacity = "0.65";
      if (ui.progress) {
        ui.progress.textContent = "🖼️ Karte, Quellenleiste und Sidecar werden erzeugt…";
      ...
      try {
        ...
        if (typeof rootValue.CustomEvent === "function" && rootValue.dispatchEvent) {
          rootValue.dispatchEvent(new rootValue.CustomEvent("unfallwerkbank:static-map-exported", {
            detail: {
    ...
    UA.exportMapPngPackage = (ctx, options) => exportCurrentMap(UA, rootValue, ctx, options);
    UA.__staticMapExportProvenanceInstalled = true;
    UA.staticMapExportProvenanceRuntime = Object.freeze({
      available: true,
      button: ui.button,
      exportCurrentMap: (ctx, options) => exportCurrentMap(UA, rootValue, ctx, options),
    });
