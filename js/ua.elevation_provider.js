...
(function initElevationProvider(root, factory) {
...
})(typeof window !== 'undefined' ? window : null, function createElevationProviderApi() {
  'use strict';
  ...
  function requiredString(value, path) {
    if (typeof value !== 'string' || !value.trim()) fail('invalid_descriptor', `${path} must be a non-empty string`);
    return value.trim();
  }
  ...
  function httpsUrl(value, path, optional) {
    ...
    let parsed;
    try { parsed = new URL(text); } catch (_) { fail('invalid_descriptor', `${path} must be an absolute URL`); }
    if (parsed.protocol !== 'https:') fail('invalid_descriptor', `${path} must use https`);
    parsed.hash = '';
  ...
  function isoDate(value, path) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}(?:T.*)?$/.test(text) || !Number.isFinite(Date.parse(text))) {
      fail('invalid_descriptor', `${path} must be ISO-8601`);
    }
    return text;
  }
  ...
  function normalizeDescriptor(value) {
    ...
    if (!MODEL_TYPES.includes(modelType)) fail('invalid_descriptor', `unsupported modelType ${modelType}`);
    return Object.freeze({
      ...
      ...(value.distributionUrl ? { distributionUrl: httpsUrl(value.distributionUrl, 'distributionUrl') } : {}),
      ...
      requiredAttribution: requiredString(value.requiredAttribution, 'requiredAttribution'),
      resolutionMeters,
      modelType,
      horizontalCrs: requiredString(value.horizontalCrs, 'horizontalCrs'),
      ...(value.verticalDatum ? { verticalDatum: requiredString(value.verticalDatum, 'verticalDatum') } : {}),
      ...(value.acquisitionPeriod ? { acquisitionPeriod: requiredString(value.acquisitionPeriod, 'acquisitionPeriod') } : {}),
      ...(value.publicationDate ? { publicationDate: requiredString(value.publicationDate, 'publicationDate') } : {}),
      retrievedAt: isoDate(value.retrievedAt, 'retrievedAt'),
      ...(value.modifiedDataNotice ? { modifiedDataNotice: requiredString(value.modifiedDataNotice, 'modifiedDataNotice') } : {}),
      priority,
    });
  }
  ...
  function createProvider(options) {
    ...
    if (typeof opts.sampleElevations !== 'function') {
      fail('invalid_provider', `${descriptor.id} must implement sampleElevations(coordinates, context)`);
    }
    const canProvide = typeof opts.canProvide === 'function' ? opts.canProvide : () => true;
    return Object.freeze({
      id: descriptor.id,
      descriptor,
      async canProvide(context) { return Boolean(await canProvide(context || {})); },
      async sampleElevations(coordinates, context) {
        const result = await opts.sampleElevations(coordinates, context || {});
        if (!Array.isArray(result) || result.length !== coordinates.length) {
          fail('invalid_samples', `${descriptor.id} returned ${result && result.length} samples for ${coordinates.length} coordinates`);
        }
        return result.map((value, index) => {
          if (value == null) return null;
          const number = Number(typeof value === 'object' ? value.elevation : value);
          if (!Number.isFinite(number)) fail('invalid_samples', `${descriptor.id} sample ${index} is not finite`);
          return number;
        });
      },
    });
    ...
    return Object.freeze({
      register(provider) {
        if (!provider || !provider.descriptor || typeof provider.canProvide !== 'function' ||
          ...
          fail('invalid_provider', 'provider does not implement the elevation contract');
        }
        if (entries.has(provider.id)) fail('duplicate_provider', `provider ${provider.id} is already registered`);
  ...
  function preparePolyline(geometry, anchorValue) {
    if (!Array.isArray(geometry) || geometry.length < 2) fail('invalid_geometry', 'road geometry requires at least two points');
    const anchor = normalizeCoordinate(anchorValue, 'anchor');
    const projection = localProjection(anchor);
    const coordinates = geometry.map((value, index) => normalizeCoordinate(value, `geometry[${index}]`));
    ...
    if (cumulative[cumulative.length - 1] < 2) fail('invalid_geometry', 'road geometry is too short');
  ...
  function coordinateAtDistance(prepared, distance) {
    ...
    for (let index = 0; index < prepared.cumulative.length - 1; index += 1) {
      if (bounded <= prepared.cumulative[index + 1]) { segment = index; break; }
    }
  ...
  function buildSamplePlan(prepared, windowMeters, spacingMeters) {
    ...
    if (end - start < Math.max(5, spacingMeters * 2)) {
      fail('insufficient_geometry', 'road geometry does not cover the requested analysis window');
    }
    const distances = [];
    for (let distance = start; distance <= end + 1e-6; distance += spacingMeters) distances.push(distance);
    if (distances[distances.length - 1] < end - spacingMeters * 0.25) distances.push(end);
  ...
  function theilSen(samples) {
    ...
    for (let left = 0; left < samples.length - 1; left += 1) {
      for (let right = left + 1; right < samples.length; right += 1) {
        ...
        slopes.push((samples[right].elevation - samples[left].elevation) / delta);
      }
    }
    if (!slopes.length) fail('insufficient_samples', 'no independent elevation sample pairs');
    const slope = median(slopes);
    const intercept = median(samples.map(sample => sample.elevation - slope * sample.alongMeters));
    const residuals = samples.map(sample => Math.abs(sample.elevation - (intercept + slope * sample.alongMeters)));
  ...
  function riskReasons(tags) {
    const value = tags || {};
    const reasons = [];
    if (String(value.bridge || '').toLowerCase() === 'yes') reasons.push('bridge_surface_not_represented_by_dtm');
    if (String(value.tunnel || '').toLowerCase() === 'yes') reasons.push('tunnel_surface_not_represented_by_dtm');
    if (Number(value.layer || 0) !== 0) reasons.push('non_ground_osm_layer');
    if (value.embankment) reasons.push('embankment_may_differ_from_terrain');
    if (value.cutting) reasons.push('cutting_may_differ_from_terrain');
    return reasons;
  }

  function qualityFor(descriptor, sampleCount, residualMadMeters, matchQuality, reasons) {
    if (reasons.some(reason => /bridge|tunnel/.test(reason))) return 'unusable';
    let score = 0;
  ...
  function displayStatement(result) {
    if (!result.usable) {
      return `Keine belastbare Straßenlängsneigung: ${result.uncertaintyReasons.join(', ')}.`;
    }
    ...
    return `Straßenlängsneigung: ${value} % ${direction} über ${result.windowMeters * 2} m, ` +
  ...
  async function computeRoadGradient(provider, geometry, anchor, options) {
    if (!provider || !provider.descriptor || typeof provider.sampleElevations !== 'function') {
      fail('invalid_provider', 'computeRoadGradient requires a registered provider');
    }
    const opts = options || {};
    const windowMeters = Math.max(10, Number(opts.windowMeters) || 50);
    const spacingMeters = Math.max(1, Number(opts.spacingMeters) || provider.descriptor.resolutionMeters);
    const prepared = preparePolyline(geometry, anchor);
    const plan = buildSamplePlan(prepared, windowMeters, spacingMeters);
    const elevations = await provider.sampleElevations(plan.map(item => item.coordinate), opts.context || {});
    const samples = plan.map((item, index) => ({
      ...item,
      elevation: elevations[index],
    })).filter(item => Number.isFinite(item.elevation));
    if (samples.length < 3) fail('insufficient_samples', `only ${samples.length} valid elevation samples`);
    const fit = theilSen(samples);
    const reasons = riskReasons(opts.osmTags);
    if (provider.descriptor.modelType !== 'DTM') reasons.push('source_is_not_pure_dtm');
    if (provider.descriptor.resolutionMeters >= 20) reasons.push('coarse_global_elevation_model');
    if (opts.matchQuality === 'low') reasons.push('low_road_match_quality');
    if (prepared.nearest.distanceSquared > 25 * 25) reasons.push('accident_point_far_from_matched_road');

    const rawGradientPercent = fit.slope * 100;
    const uncertaintyPercent = Math.max(
      provider.descriptor.resolutionMeters >= 20 ? 1 : 0.2,
      100 * 1.4826 * fit.residualMadMeters / Math.max(windowMeters, 1)
    );
    ...
    const semanticType = provider.descriptor.resolutionMeters >= 20 || provider.descriptor.modelType !== 'DTM'
      ? 'terrain_context'
      : 'road_longitudinal_gradient';
    if (semanticType === 'terrain_context') quality = degradeQuality(quality);
    const unusable = quality === 'unusable';
    const gradientPercent = unusable ? null :
      semanticType === 'terrain_context' ? Math.round(rawGradientPercent) : Math.round(rawGradientPercent * 10) / 10;
    const range = unusable ? null : [
    ...
    const result = {
      ...
      matchDistanceMeters: Math.round(Math.sqrt(prepared.nearest.distanceSquared) * 10) / 10,
  ...
  function createHannoverDgm1Descriptor(retrievedAt) {
    return normalizeDescriptor({
      ...
      datasetUrl: 'https://www.hannover.de/Leben-in-der-Region-Hannover/Verwaltungen-Kommunen/Die-Verwaltung-der-Landeshauptstadt-Hannover/Dezernate-und-Fachbereiche-der-LHH/Stadtentwicklung-und-Bauen/Fachbereich-Planen-und-Stadtentwicklung/Geoinformation/Open-GeoData/3D-Stadtmodell-und-Gel%C3%A4ndemodell/Digitales-Gel%C3%A4ndemodell-DGM1',
      licenseId: 'CC-BY-4.0',
      licenseName: 'Creative Commons Attribution 4.0 International',
      licenseUrl: 'https://creativecommons.org/licenses/by/4.0/',
      requiredAttribution: '© Landeshauptstadt Hannover, Fachbereich Planen und Stadtentwicklung, Geoinformation, CC BY 4.0',
      resolutionMeters: 1,
      modelType: 'DTM',
      horizontalCrs: 'EPSG:25832',
      retrievedAt,
      modifiedDataNotice: 'Höhenprofil entlang gematchter Straßengeometrien robust ausgewertet.',
  ...
  function createGlobalFallbackDescriptor(retrievedAt) {
    return normalizeDescriptor({
      ...
      retrievedAt,
      modifiedDataNotice: 'Global raster values sampled and converted to a coarse terrain-context profile.',
