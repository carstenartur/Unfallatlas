...
function bindRecordedContextColors(buffer, width, height, requiredState, frameEvidence) {
    ...
    for (const kind of requestedKinds) {
      ...
      for (let y = minY; y <= maxY; y++) {
        for (let x = minX; x <= maxX; x++) {
          ...
          const index = start + (y * width + x) * 4;
          const color = [buffer[index], buffer[index + 1], buffer[index + 2]];
          const distance = channelDistance(color, styleColor);
          if (!candidate.best || distance < candidate.best.distance) {
            candidate.best = { color, distance, frame, x, y };
          }
          if (distance <= SOURCE_STYLE_MAX_DISTANCE) candidate.qualifyingPixels += 1;
        }
      }
    }
  }
  ...
  for (const kind of requestedKinds) {
    const candidate = candidates[kind];
    if (!candidate.best || candidate.best.distance > SOURCE_STYLE_MAX_DISTANCE ||
        candidate.qualifyingPixels < SOURCE_STYLE_MIN_PIXELS) {
      throw Object.assign(new Error(
        ...
        details: {
          evidenceFrameCount,
          best: candidate.best,
          qualifyingPixels: candidate.qualifyingPixels,
    ...
    enriched.contextWitnesses[kind] = {
      ...candidate.witness,
      renderedColor: candidate.best.color,
      renderedStyleDistance: candidate.best.distance,
      renderedQualifyingPixels: candidate.qualifyingPixels,
      renderedSourceFrame: candidate.best.frame,
      renderedSourcePoint: { x: candidate.best.x, y: candidate.best.y },
    };
  }
