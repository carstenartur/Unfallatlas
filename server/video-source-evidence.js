...
function bindRecordedContextColors(buffer, width, height, requiredState, frameEvidence) {
  ...
  for (let frame = 0; frame < frameCount; frame++) {
    ...
    if (sharedCompositeContext &&
      ...
      // The first qualifying frame is inside the explicit pre-dialog hold after
      // the badge and geometry-bound rings are installed. Later frames may be
      // covered by the export modal and must never replace this real map proof
      // merely because an unrelated modal pixel is closer to a CSS style colour.
      commonContextFrame = { frame, results: frameResults };
      break;
    }
  }
