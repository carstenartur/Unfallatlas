...
describe('UA.cleanUrlIfNeeded', () => {
  test('accepts map/context params as canonical (no unknown/reload)', () => {
    const href = 'http://localhost/werkbank_v2.html?city=Bonn&mapLayer=slope,traffic&ctxSlope=steep,very_steep&ctxTraffic=high&ctxOnlyMatched=1';
  ...
  test('canonical rebuild keeps map/context/debug params in CANON order', () => {
    const href = 'http://localhost/werkbank_v2.html?city=Berlin&city=Bonn&mapLayer=slope,traffic&ctxSlope=steep,very_steep&ctxTraffic=high&ctxOnlyMatched=1&debugSlope=1&debugSlopeSamples=1';
    const win = loadState(href);
    ...
    expect(cleaned.search.slice(1)).toBe(
      'city=Bonn&ctxSlope=steep%2Cvery_steep&ctxTraffic=high&ctxOnlyMatched=1&mapLayer=slope%2Ctraffic&debugSlope=1&debugSlopeSamples=1'
    );
  });
});
