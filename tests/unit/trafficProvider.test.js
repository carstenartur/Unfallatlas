...
function sourceFields(overrides = {}) {
  return {
    ...
    changeNotice:
      "Auf relevante Zählstellen gefiltert und auf Straßenabschnitte gematcht.",
    permissions: {
...
function descriptor(measurementType, overrides = {}) {
  return {
    ...
    priority:
      measurementType === "count" ? 1 : measurementType === "model" ? 2 : 4,
    ...overrides,
  };
}
...
describe("licensed traffic evidence provider core", () => {
  ...
  test("normalizes a measured observation with complete linked provenance", async () => {
    ...
    expect(observations[0].source).toEqual(
      expect.objectContaining({
  ...
  test("registry collects only available providers in deterministic priority order", async () => {
    ...
    const proxy = createProvider("proxy", [
      traffic.createOsmProxyObservation({
        observationId: "proxy-way-2",
        highway: "secondary",
        wayId: "2",
        year: 2026,
      }),
    ]);
    const measured = createProvider("count", [
      rawObservation({ wayId: "1", geometry: undefined }),
    ]);
    const unavailableModel = createProvider(
      "model",
      [
        rawObservation({
          observationId: "model-unavailable",
  ...
  test("keeps an explicit way ID without geometry but does not invent a distance", async () => {
    const [observation] = await createProvider("count", [
      rawObservation({
        wayId: "42",
        geometry: undefined,
      }),
    ]).loadObservations();
    const match = traffic.matchObservationToRoads(observation, [
      road("7"),
      road("42"),
    ]);
  ...
  test("rejects an explicit way ID whose geometry is spatially inconsistent", async () => {
    const [observation] = await createProvider("count", [
      rawObservation({
        wayId: "42",
        geometry: pointGeometry(BONN.lat + 0.01, BONN.lon),
      }),
    ]).loadObservations();
    expect(
      traffic.matchObservationToRoads(observation, [road("42")], {
  ...
  test("matches point counts to the nearest road and reports distance quality", async () => {
    ...
    expect(
      traffic.matchObservationToRoads(observation, [road("too-far", 0.01)], {
        maxDistanceMeters: 100,
      }),
    ).toBeNull();
  });
  ...
  test("prefers a fresh measured count over model, stale count and proxy", async () => {
    const countProvider = createProvider("count", [
      rawObservation({ wayId: "1", geometry: undefined }),
    ]);
    ...
    const proxyProvider = createProvider("proxy", [
      traffic.createOsmProxyObservation({
        observationId: "proxy-1",
        highway: "primary",
        wayId: "1",
        year: 2026,
      }),
    ...
    expect(result.statement).toMatch(
      /Gemessene Verkehrsbelastung: 18[.\s]500 Kfz\/24 h/,
    );
    expect(result.statement).toMatch(/DTVw 2024/);
  });

  test("prefers a current model over an outdated measured count", async () => {
    const staleProvider = createProvider("count", [
      rawObservation({
        observationId: "count-2010",
        year: 2010,
        value: 13000,
        wayId: "1",
        geometry: undefined,
  ...
  test("labels an old measured value and preserves its year", async () => {
    const provider = createProvider("count", [
      rawObservation({
        observationId: "count-2012",
        year: 2012,
        value: 12500,
        wayId: "1",
        geometry: undefined,
  ...
  test("rejects numeric proxy values and proxy units fail closed", async () => {
    expect(() =>
      traffic.normalizeDescriptor(descriptor("proxy", { unit: "Kfz/24 h" })),
    ).toThrow(/proxy_numeric_unit_forbidden/);

    const provider = createProvider("proxy", [
      {
        ...traffic.createOsmProxyObservation({
          observationId: "bad-proxy",
          highway: "primary",
          wayId: "1",
          year: 2026,
        }),
    ...
    await expect(provider.loadObservations()).rejects.toThrow(
  ...
  test("rejects descriptor/observation type, mode, unit and location mismatches", async () => {
    const wrongType = createProvider("count", [
      rawObservation({ measurementType: "model" }),
    ]);
    await expect(wrongType.loadObservations()).rejects.toThrow(
      /observation_type_mismatch/,
    );

    const wrongMode = createProvider("count", [
      rawObservation({ mode: "bicycle" }),
    ]);
    await expect(wrongMode.loadObservations()).rejects.toThrow(
      /observation_mode_mismatch/,
    );

    const wrongUnit = createProvider("count", [
      rawObservation({ unit: "Fahrzeuge/Stunde" }),
    ]);
    await expect(wrongUnit.loadObservations()).rejects.toThrow(
      /observation_unit_mismatch/,
    );

    const noLocation = createProvider("count", [
      rawObservation({ geometry: undefined, wayId: undefined }),
    ]);
    await expect(noLocation.loadObservations()).rejects.toThrow(
      /missing_location/,
    );
  });

  test("keeps modes separate when selecting evidence", async () => {
    const motorProvider = createProvider("count", [
      rawObservation({
        observationId: "motor",
        wayId: "1",
        geometry: undefined,
    ...
    const bicycleProvider = createProvider(
      "count",
      [
        {
          ...rawObservation({
    ...
    expect(
      traffic.selectTrafficEvidence(matches, {
        referenceYear: 2026,
        mode: "bicycle",
  ...
  test("does not use observations from after the selected reference year", async () => {
    const futureProvider = createProvider("count", [
      rawObservation({
        observationId: "future-count",
        year: 2028,
        wayId: "1",
        geometry: undefined,
      }),
    ]);
    const currentModel = createProvider("model", [
      rawObservation({
        observationId: "current-model",
        measurementType: "model",
        year: 2026,
        wayId: "1",
        geometry: undefined,
