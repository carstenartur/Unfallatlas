...
describe("UA.readPngDimensions / UA.fitWithAspectRatio / UA.fitImageToMax", () => {
  ...
  beforeEach(() => {
    ...
    (function (window) {
      eval(fs.readFileSync(reportPath, "utf8"));
    })(win);
    UA = win.UA;
  });
  ...
  test("readPngDimensions parses IHDR width/height correctly", () => {
    expect(UA.readPngDimensions(syntheticPng(800, 600))).toEqual({
      width: 800,
      height: 600,
    });
    expect(UA.readPngDimensions(syntheticPng(1024, 512))).toEqual({
      width: 1024,
      height: 512,
    });
  ...
  test("fitWithAspectRatio uses the SAME scale on both axes", () => {
    ...
    const r1 = UA.fitWithAspectRatio(
    ...
    const r2 = UA.fitWithAspectRatio(
    ...
    const r3 = UA.fitWithAspectRatio(
  ...
  test("fitImageToMax falls back to max-box ratio (not raw dimensions) when PNG header cannot be parsed", () => {
    ...
    expect(UA.fitImageToMax("SGVsbG8=", UA.DOCX_MAP_MAX)).toEqual({
      width: 600,
      height: 400,
    });
    expect(UA.fitImageToMax("", UA.PDF_MAP_MAX)).toEqual({
      width: 475,
      height: 340,
    });
  });
  ...
  test("fitImageToMax with strict:true throws when PNG header cannot be parsed", () => {
    expect(() =>
      UA.fitImageToMax("SGVsbG8=", UA.DOCX_MAP_MAX, { strict: true }),
    ).toThrow();
    expect(() =>
      UA.fitImageToMax("", UA.PDF_MAP_MAX, { strict: true }),
    ).toThrow();
  });

  test("fitImageToMax with fallbackRatio: 16/9 produces correct ratio within tolerance", () => {
    const result = UA.fitImageToMax("SGVsbG8=", UA.DOCX_MAP_MAX, {
      fallbackRatio: 16 / 9,
    });
    const expectedRatio = 16 / 9;
    const actualRatio = result.width / result.height;
    expect(Math.abs(actualRatio - expectedRatio)).toBeLessThan(
      UA.ASPECT_TOLERANCE,
    );
...
describe("DOCX/PDF export: aspect-ratio regression", () => {
  ...
  test("PDF: every map uses fit:[PDF_MAP_MAX.width, PDF_MAP_MAX.height]", async () => {
    ...
    await UA.exportToPDF(makeFixtureCtx(), fixtureReport, {
      _skipQAGate: true,
      includeMap: true,
    });
  ...
  test("PDF: image and caption rendered as unbreakable stack (block stays together)", async () => {
    ...
    const unbreakableImageStacks = captured.content.filter(
      (n) =>
        n &&
        typeof n === "object" &&
        n.unbreakable === true &&
    ...
    for (const s of unbreakableImageStacks) {
      ...
      const hasCaption = s.stack.some(
        (n) =>
          n && typeof n.text === "string" && /Abbildung\s*\d+:/.test(n.text),
      );
  ...
  test("DOCX: ImageRun.transformation preserves PNG aspect ratio for all map types", async () => {
    ...
    jest.spyOn(docx, "ImageRun").mockImplementation(function (opts) {
      if (opts && opts.transformation)
        transformations.push(opts.transformation);
      return new OrigImageRun(opts);
    });

    await UA.exportToWord(makeFixtureCtx(), fixtureReport, {
      includeMap: true,
    });
    ...
    for (let i = 0; i < cases.length; i++) {
      ...
      expect(Math.abs(afterRatio - beforeRatio)).toBeLessThan(
        UA.ASPECT_TOLERANCE,
      );
      ...
      expect(
        after.width === UA.DOCX_MAP_MAX.width ||
          after.height === UA.DOCX_MAP_MAX.height,
      ).toBe(true);
    }
  });
  ...
  test("DOCX: image paragraph carries keepNext (Bild + Caption als Einheit)", async () => {
    ...
    const imageParagraphs = paragraphProps.filter(
      (p) =>
        Array.isArray(p.children) &&
        p.children.some(
          (c) => c && c.constructor && c.constructor.name === "ImageRun",
        ),
    );
