...
describe('recorded source-frame evidence', () => {
  ...
  test('selects the earliest distinguishable same-frame pair before later overlays', () => {
    ...
    // Frame 2 is the first genuine same-frame road composite.
    for (const point of [[7, 8], [8, 8], [9, 8]]) {
    ...
    // A later modal/overlay frame can contain numerically closer colours but
    // must never replace the first pre-dialog map proof.
    for (const point of [[7, 8], [8, 8], [9, 8]]) {
      setPixel(buffer, width, height, 3, point[0], point[1], [240, 59, 32]);
    }
    for (const point of [[8, 7], [8, 9], [9, 9]]) {
      setPixel(buffer, width, height, 3, point[0], point[1], [39, 125, 161]);
    }
