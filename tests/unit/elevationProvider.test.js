...
const ANCHOR = { lat: 52.3759, lon: 9.7320 };
...
describe('provider-based elevation and road-gradient core', () => {
  test('exposes the identical browser and Node API without Leaflet', () => {
    ...
    expect(elevation.SOURCE_TIERS.OFFICIAL_DGM_1_2)
      .toBeLessThan(elevation.SOURCE_TIERS.GLOBAL_FALLBACK);
  });
  ...
  test('registry resolves the highest-priority available provider deterministically', async () => {
    ...
    const fallback = linearProvider({
      descriptor: { id: 'fallback', priority: 4, resolutionMeters: 30, modelType: 'mixed' },
    });
    const dgm1 = linearProvider({ descriptor: { id: 'dgm1', priority: 1, resolutionMeters: 1 } });
    ...
    expect(registry.list().map(provider => provider.id)).toEqual(['unavailable', 'dgm1', 'fallback']);
  ...
  test('computes a robust longitudinal gradient along the matched road', async () => {
    ...
    expect(result.statement).toMatch(/Straßenlängsneigung: 5,0 % bergauf über 100 m/);
  ...
  test('normalizes direction relative to the supplied road geometry', async () => {
    ...
    const forward = await elevation.computeRoadGradient(provider, eastWestRoad(), ANCHOR, {
      windowMeters: 40, spacingMeters: 4, matchQuality: 'high',
    });
    const reverse = await elevation.computeRoadGradient(provider, eastWestRoad().slice().reverse(), ANCHOR, {
      windowMeters: 40, spacingMeters: 4, matchQuality: 'high',
    });
  ...
  ])('does not claim a road gradient for unsupported structures %p', async (osmTags, reason) => {
    const result = await elevation.computeRoadGradient(
      linearProvider(),
      eastWestRoad(),
      ANCHOR,
  ...
  test('degrades embankment, cutting and poor matching instead of hiding uncertainty', async () => {
    ...
    expect(result.uncertaintyReasons).toEqual(expect.arrayContaining([
  ...
  test('labels a coarse global model as terrain context without decimal precision', async () => {
    ...
    const result = await elevation.computeRoadGradient(fallback, eastWestRoad(180), ANCHOR, {
      windowMeters: 60,
      spacingMeters: 30,
      matchQuality: 'high',
    });
  ...
  test('fails closed on invalid descriptors and incomplete sample arrays', async () => {
    expect(() => elevation.createProvider({
    ...
    await expect(elevation.computeRoadGradient(provider, eastWestRoad(), ANCHOR, {
      windowMeters: 40,
      spacingMeters: 4,
  ...
  test('fails closed when geometry or source coverage cannot support a profile', async () => {
    ...
    await expect(elevation.computeRoadGradient(linearProvider(), [ANCHOR], ANCHOR))
  ...
  test('Hannover DGM1 descriptor contains the required official provenance', () => {
    ...
    expect(dgm).toEqual(expect.objectContaining({
      id: 'hannover.dgm1',
