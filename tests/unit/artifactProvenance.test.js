...
function manifest(overrides = {}) {
  return {
    ...
    scenario: {
      ...
      filters: { involvementMode: 'and', includeCyclist: true, includeCar: true },
...
describe('artifact provenance format adapters', () => {
  ...
  test('hashes canonical normalized manifest JSON deterministically', async () => {
    ...
    const second = await provenance.normalizeAndHash(manifest({
  ...
  test('creates CSV ZIP entries with CSV, full sources JSON and linked README', async () => {
    ...
    expect(result.entries.map(entry => entry.name)).toEqual([
      'Bonn-Hbf-Rad-Pkw.csv', 'sources.json', 'README.txt',
    ]);
    ...
    expect(result.entries[2].content).toContain(`SourceManifest SHA-256: ${result.sourceManifestSha256}`);
  ...
  test('rejects empty CSV content and unsafe/empty package names', async () => {
    await expect(provenance.buildCsvPackageEntries({
      baseName: 'valid', csv: '', manifest: manifest(),
  ...
  test('embeds the complete manifest and source IDs in GeoJSON', async () => {
    const input = {
      ...
      features: [
        {
          type: 'Feature',
          properties: { id: 1, 'unfallatlas:sourceIds': ['accidents.de.unfallatlas'] },
    ...
    expect(result.geojson.metadata['unfallatlas:sourceManifestSha256'])
      .toBe(result.sourceManifestSha256);
    expect(result.geojson.features[0].properties['unfallatlas:sourceIds'])
      .toEqual(['accidents.de.unfallatlas']);
    expect(result.geojson.features[1].properties['unfallatlas:sourceIds'])
      .toEqual(['accidents.de.unfallatlas', 'roads.openstreetmap']);
    expect(input).not.toHaveProperty('metadata');
    expect(input.features[1].properties).not.toHaveProperty('unfallatlas:sourceIds');
  });
  ...
  test('fails closed on unknown feature Source-IDs or duplicate metadata', async () => {
    ...
    await expect(provenance.attachGeoJsonProvenance(unknown, manifest()))
    ...
    await expect(provenance.attachGeoJsonProvenance(duplicate, manifest()))
      .rejects.toThrow(/existing_provenance/);
  });

  test('rejects non-FeatureCollection or malformed Feature entries', async () => {
    await expect(provenance.attachGeoJsonProvenance({ type: 'Feature' }, manifest()))
      .rejects.toThrow(/invalid_geojson/);
    await expect(provenance.attachGeoJsonProvenance({
      type: 'FeatureCollection', features: [{}],
    }, manifest())).rejects.toThrow(/invalid_geojson/);
  });
  ...
  test('injects KML provenance exactly once into Document', async () => {
    const original = '<?xml version="1.0"?><kml><Document><name>Bonn &amp; Hbf</name></Document></kml>';
    ...
    await expect(provenance.injectKmlProvenance(first.kml, manifest()))
      .rejects.toThrow(/existing_provenance/);
    await expect(provenance.injectKmlProvenance('<kml/>', manifest()))
  ...
  test('accepts a precomputed artifact hash but rejects malformed hashes', async () => {
    ...
    await expect(provenance.buildSidecar({
      artifactName: 'data.geojson',
      artifactMediaType: 'application/geo+json',
      artifactSha256: 'abc',
      manifest: manifest(),
  ...
  test('does not accept format-specific source IDs that are absent from SourceManifest', async () => {
    await expect(provenance.attachGeoJsonProvenance({
      type: 'FeatureCollection',
      features: [],
    }, manifest(), { defaultSourceIds: ['format.local.source'] }))
      .rejects.toThrow(/unknown_source_id/);
  });
});
