...
function accidentSource(overrides = {}) {
  return {
    ...
    changeNotice: 'Gefiltert, reprojiziert und für die Unfallwerkbank zusammengeführt.',
    permissions: {
...
function osmSource(overrides = {}) {
  return {
    ...
    changedOrDerived: true,
    changeNotice: 'Straßengeometrien und Tags räumlich gefiltert und abgeleitete Klassen erzeugt.',
    ...overrides,
  };
}
...
function validManifest(overrides = {}) {
  return {
    ...
    scenario: {
      city: 'Bonn',
      bounds: { south: 50.73, west: 7.09, north: 50.74, east: 7.10 },
    ...
    transformations: [
      {
        ...
        parameters: { involvementMode: 'and', includeCyclist: true, includeCar: true },
      },
      {
...
describe('renderer-independent source manifest', () => {
  test('normalizes one deterministic immutable manifest for every renderer', () => {
    ...
    expect(normalized.transformations.map(item => item.transformationId)).toEqual([
      'transform.accident-filter',
      'transform.road-context',
    ]);
    expect(Object.isFrozen(normalized)).toBe(true);
    expect(Object.isFrozen(normalized.sources[0])).toBe(true);
    expect(sourceManifest.stableStringify(normalized))
      .toBe(sourceManifest.stableStringify(sourceManifest.normalizeManifest(validManifest({
  ...
  test('publishes the same browser and Node API', () => {
    ...
    expect(sourceManifest.LICENSE_POLICIES['DL-DE-BY-2.0'].requiresAttribution).toBe(true);
  });

  test('canonicalizes accepted licence aliases but not arbitrary licences', () => {
    const normalized = sourceManifest.normalizeManifest(validManifest({
      sources: [accidentSource({ licenseId: 'dl-de/by-2-0' }), osmSource({ licenseId: 'ODbL 1.0' })],
    }));
    ...
    expect(() => sourceManifest.normalizeManifest(validManifest({
      sources: [accidentSource({ licenseId: 'Custom-Portal-Terms' }), osmSource()],
  ...
  ])('fails closed when required source field %s is invalid', (field, value) => {
    expect(() => sourceManifest.normalizeManifest(validManifest({
  ...
  test('rejects duplicate and orphaned source references', () => {
    ...
    expect(() => sourceManifest.normalizeManifest(manifest))
      .toThrow(/orphaned_source_reference/);
  });
  ...
  test('rejects duplicate transformation IDs and unknown fields', () => {
    ...
    expect(() => sourceManifest.normalizeManifest(duplicate))
      .toThrow(/duplicate_transformation_id/);

    expect(() => sourceManifest.normalizeManifest({ ...validManifest(), debug: true }))
  ...
  test('rejects invalid fingerprints, artifact hashes, bounds and years', () => {
    expect(() => sourceManifest.normalizeManifest(validManifest({ buildFingerprint: 'abc' })))
      .toThrow(/invalid_sha256/);
    expect(() => sourceManifest.normalizeManifest(validManifest({ artifactHash: 'abc' })))
      .toThrow(/invalid_sha256/);
    expect(() => sourceManifest.normalizeManifest(validManifest({
      scenario: { ...validManifest().scenario, bounds: { south: 51, west: 7, north: 50, east: 8 } },
  ...
  test('derives field-specific source IDs only through declared transformations', () => {
    const manifest = validManifest();
    expect(sourceManifest.sourceIdsForFields(manifest, ['severityCounts']))
      .toEqual(['accidents.de.unfallatlas']);
    expect(sourceManifest.sourceIdsForFields(manifest, ['roadContext']))
      .toEqual(['accidents.de.unfallatlas', 'roads.openstreetmap']);
    expect(sourceManifest.sourceIdsForFields(manifest, ['unknown']))
      .toEqual([]);
  });
