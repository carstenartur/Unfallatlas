...
function normalizeImagePath(sourceFile, imagePath) {
  const absolute = path.posix.normalize(
    path.posix.join(path.posix.dirname(sourceFile.replace(/\\/g, '/')), imagePath),
  );
...
function assertCanonicalUrl(link, scenario) {
  ...
  const unexpected = [...url.searchParams.keys()].filter((key) => !(key in expected));
...
function loadDocumentationLinks(rootDir) {
  const files = ['README.md', 'docs/DOKUMENTATION.md'];
  return Object.freeze(files.flatMap((sourceFile) => {
    const absolute = path.join(rootDir, sourceFile);
    return extractLinkedScreenshots(fs.readFileSync(absolute, 'utf8'), sourceFile);
  }));
}

function validateDocumentationLinks(rootDir = process.cwd()) {
  const links = loadDocumentationLinks(rootDir);
  ...
  for (const [imagePath, scenario] of Object.entries(SCENARIOS)) {
    const imageLinks = byImage.get(imagePath) || [];
    if (scenario.requiredInReadme && !imageLinks.some((link) => link.sourceFile === 'README.md')) {
      fail('missing_readme_screenshot_link', `${imagePath} is missing from README.md`);
    }
    const targets = new Set(imageLinks.map((link) => link.url));
    if (targets.size > 1) {
      fail('inconsistent_documentation_targets', `${imagePath} links to multiple live states`, {
        links: imageLinks,
      });
    }
  }
  ...
  const liveScenarios = Object.entries(SCENARIOS).map(([imagePath, scenario]) => {
    const imageLinks = byImage.get(imagePath) || [];
    const readmeLink = imageLinks.find((link) => link.sourceFile === 'README.md');
